...
class ITR_GP_RTO:
    ...
    def Ellipse_sampling(self, H_norm, z_hat, m_FA, Gamma_Threshold=1.0):
        '''
        This function samples randomly withing an ellipse self.Delta0
        '''
        H_norm_abs = np.sqrt(np.abs(np.diag(H_norm)))
    ...
    def Step_constraint(self, Delta0, xk, xnew, GP_obj, i_rto):
        ...
        if not self.System_Cons_check(np.array(xnew).flatten()):
            ...
            print('D', xnew)
            # print('new', Delta0 * self.gamma_red)
    ...
    def RTO_routine(self):
        ...
        for i_rto in range(n_iter):
            ...
            for j in range(multi_opt):
                ...
                localsol[j] = res.x
                # print(GP_obj.GP_inference_np((d_init + xk)))
                # print(GP_con_f[0]['fun'](d_init))
                # print(GP_con_f[1]['fun'](d_init))
                # print(GP_con_f[2]['fun'](d_init))
                # print(d_init)
                # print('sol', res.x)
                # print('sol', TRb)
                # print('obj', GP_obj_f(d_init, GP_obj, xk))
